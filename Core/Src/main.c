...
/* ------------------ Обробка GPIO-переривань ------------------ */
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  if (GPIO_Pin == GPIO_PIN_15) // Кнопка збільшення
  {
    if (leds_count < 4)
    {
      HAL_GPIO_WritePin(GPIOA, LEDS[leds_count], GPIO_PIN_SET);
      leds_count++;
    }
  }
  else if (GPIO_Pin == GPIO_PIN_14) // Кнопка зменшення
  {
    if (leds_count > 0)
    {
      leds_count--;
      HAL_GPIO_WritePin(GPIOA, LEDS[leds_count], GPIO_PIN_RESET);
    }
  }
}
/* USER CODE END 4 */
